    ...
    it('should use UserService.update() to update a user', function (done) {
      var data = {
        ...
        , res = fakeResponse(function(status, result) {
          ...
          expect(spyCall[0]).to.be.an('object');
          expect(spyCall[0]).to.have.property('firstName').and.equal(data.firstName);
          expect(spyCall[1]).to.be.an('object').and.to.have.property('where').and.to.have.property('id').and.equal(data.id);
    ...
    it('should update the logged in users password', function(done) {
      var data = {
        ...
        , res = fakeResponse(function(status, result) {
          ...
          expect(spyCall).to.be.an('array');
          expect(spyCall[0]).to.be.an('object');
          expect(spyCall[0]).to.have.property('password');
          expect(spyCall[0]).to.not.have.property('new_password');
          expect(spyCall[1]).to.be.an('object').and.to.have.property('where').and.to.have.property('id').and.equal(new_user.id);
